...
import { aliasMatches, createError, createTypeVar, createVar, getFunctionNameAndArgs, isAlias, typeSatisfaction, } from "./types.ts";
import { morphTypes, registerBuiltins } from "./builtins.ts";
export const ctx = new Context();
const assignGenericTableToTypeVars = (ctx, genericTable) => {
    for (const genericName of Object.keys(genericTable)) {
        ctx.assignValue(genericName, createTypeVar(genericTable[genericName]));
    }
};
const unassignGenericTableToTypeVars = (ctx, genericTable) => {
    for (const genericName of Object.keys(genericTable)) {
        ctx.deleteValue(genericName);
    }
};
export function callFunctionByReference(ref, args, ctx, name) {
    ...
    for (const candidateFunction of candidates) {
        ...
        for (let i = 0; i < candidateFunction.types.length; i++) {
            ...
            const specificity = "specificities" in candidateFunction
                ? candidateFunction.specificities[i]
                : 1;
            let [typeScore, gt] = typeSatisfaction(args[i].type, type, genericTable, 0, args[i]);
            genericTable = Object.assign(Object.assign({}, genericTable), gt);
            if (typeScore === 0) {
            ...
            const intScore = typeScore * (condition(tempArgs[i], ctx) ? 1 : 0) * specificity;
            if (intScore === 0) {
                score = -1;
                break;
            }
        ...
        if (modifiedArgs.every((entry) => entry.type.baseName === "TUPLE")) {
            ...
            if (modifiedArgs.every((arg) => arg.value.length === tupleSize)) {
                ...
                for (let i = 0; i < tupleSize; i++) {
                    result.push(callFunctionByReference(ref, modifiedArgs.map((arg) => arg.value[i]), ctx, name));
                }
...
export function dispatchFunction(fnName, args) {
    ...
    if (!named) {
        ...
        if (booleanName && assertionName) {
            return createError("Ambiguous conversion from " +
                fnName +
                " to " +
                fnName +
                "? and " +
        ...
        named = booleanName !== null && booleanName !== void 0 ? booleanName : assertionName;
    }
...
export function evaluate(realNode, skipVarLookup, ignoreExpressions) {
    ...
    if (aliasMatches(realNode.type, "CALL")) {
        ...
        return dispatchFunction(name, args.map((arg, index) => evaluate(arg, isAssignment && index === 0)));
    }
    ...
    if (Array.isArray(realNode.value) &&
        ...
        realNode.value = realNode.value.map((item) => evaluate(item, skipVarLookup, true));
    }
    return realNode;
}
export function unknownVariablesInExpression(expr) {
    const foundVars = [];
...
export function parseToExpr(text) {
