import { parse, trimAndSplitOnCommas } from "./parser.ts";
...
export const callFunctionByReference = (
  ref,
  args,
  variables,
  morphisms,
  ...
  for (const candidateFunction of candidates) {
    ...
    for (let i = 0; i < candidateFunction.types.length; i++) {
      ...
      const intScore =
        typeScore *
        (condition(tempArgs[i], variables, morphisms) ? 1 : 0) *
        specificity;
      if (intScore === 0) {
        score = -1;
        break;
...
export const evaluate = (text, variables, morphisms, goal) => {
  ...
  if (isFunctionCall(text)) {
    ...
    const args = trimAndSplitOnCommas(body).map((e) =>
      evaluate(e, variables, morphisms, goal),
    );
